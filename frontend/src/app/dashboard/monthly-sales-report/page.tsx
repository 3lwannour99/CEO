    ...
    <main className={styles.page}>
      ...
      <section className={styles.filters} aria-label={t("monthlySalesReport.filters")}>
        <Field label={t("filters.fromDate")}>
          <input type="date" value={filters.dateFrom} onChange={(event) => updateDate("dateFrom", event.target.value, setFilters)} />
        </Field>
        <Field label={t("filters.toDate")}>
          <input type="date" value={filters.dateTo} onChange={(event) => updateDate("dateTo", event.target.value, setFilters)} />
        </Field>
        <Field label={t("monthlySalesReport.salesLocation")}>
          <select value={filters.salesLocation} onChange={(event) => setFilters((current) => ({ ...current, salesLocation: event.target.value }))}>
            <option value="">{t("monthlySalesReport.allLocations")}</option>
            {(report?.options.salesLocations ?? []).map((location) => <option value={location} key={location}>{translateUnmapped(location, t)}</option>)}
          </select>
        </Field>
        <Field label={t("monthlySalesReport.salesman")}>
          <select value={filters.salesman} onChange={(event) => setFilters((current) => ({ ...current, salesman: event.target.value }))}>
            <option value="">{t("monthlySalesReport.allSalesmen")}</option>
            {(report?.options.salesmen ?? []).map((salesman) => <option value={salesman} key={salesman}>{salesman}</option>)}
          </select>
        </Field>
        ...
        <Field label={t("filters.country")}>
          <select value={filters.country} onChange={(event) => setFilters((current) => ({ ...current, country: event.target.value }))}>
            ...
            {(report?.options.countries ?? []).map((country) => <option value={country} key={country}>{country}</option>)}
          </select>
        </Field>
        <Field label={t("filters.source")}>
          <select value={filters.sourceId} onChange={(event) => setFilters((current) => ({ ...current, sourceId: event.target.value }))}>
            <option value="">{t("monthlySalesReport.allSources")}</option>
            {(report?.options.sources ?? []).map((source) => <option value={source.value} key={source.value}>{source.label}</option>)}
          </select>
        </Field>
        <Field label={t("filters.branch")}>
          <select value={filters.branch} onChange={(event) => setFilters((current) => ({ ...current, branch: event.target.value }))}>
            ...
            {(report?.options.branches ?? []).map((branch) => <option value={branch} key={branch}>{branch}</option>)}
          </select>
        </Field>
        <Field label={t("filters.warehouse")}>
          <select value={filters.warehouse} onChange={(event) => setFilters((current) => ({ ...current, warehouse: event.target.value }))}>
            <option value="">{t("monthlySalesReport.allWarehouses")}</option>
            {(report?.options.warehouses ?? []).map((warehouse) => <option value={warehouse} key={warehouse}>{warehouse}</option>)}
          </select>
        </Field>
        <Field label={t("filters.model")}>
          <select value={filters.model} onChange={(event) => setFilters((current) => ({ ...current, model: event.target.value }))}>
            ...
            {(report?.options.models ?? []).map((model) => <option value={model} key={model}>{model}</option>)}
          </select>
        </Field>
        <Field label={t("filters.type")}>
          <select value={filters.type} onChange={(event) => setFilters((current) => ({ ...current, type: event.target.value }))}>
            <option value="">{t("monthlySalesReport.allTypes")}</option>
            {(report?.options.types ?? []).map((type) => <option value={type} key={type}>{type}</option>)}
          </select>
        </Field>
        <Field label={t("filters.customerGroup")}>
          <select value={filters.customerGroup} onChange={(event) => setFilters((current) => ({ ...current, customerGroup: event.target.value }))}>
            <option value="">{t("monthlySalesReport.allCustomerGroups")}</option>
            {(report?.options.customerGroups ?? []).map((group) => <option value={group} key={group}>{group}</option>)}
          </select>
        </Field>
        <Field label={t("filters.search")}>
          <input type="search" value={filters.search} onChange={(event) => setFilters((current) => ({ ...current, search: event.target.value }))} />
        </Field>
        <div className={styles.filterActions}>
          <button type="button" onClick={() => setFilters(defaultFilters())}>{t("filters.resetFilters")}</button>
      ...
      <section className={styles.reportCard}>
        <div className={styles.reportMeta}>
          <strong>{t("monthlySalesReport.period")}: {report?.dateFrom ?? filters.dateFrom} - {report?.dateTo ?? filters.dateTo}</strong>
          <span>{t("common.lastUpdated")}: {report ? new Date(report.generatedAt).toLocaleString(language) : "-"}</span>
        </div>
          ...
          <table className={styles.reportTable}>
            <thead>
              <tr>
                ...
                {BRANDS.map((brand) => <th colSpan={2} key={brand}>{brand}</th>)}
            ...
            <tbody>
              {loading && !report ? (
                <tr><td colSpan={11} className={styles.messageCell}>{t("common.loading")}</td></tr>
              ) : groupedRows.length === 0 ? (
                <tr><td colSpan={11} className={styles.messageCell}>{t("monthlySalesReport.noData")}</td></tr>
              ) : groupedRows.map(({ location, rows, total }) => {
                return [
                  ...rows.map((row, index) => (
                    <tr key={`${location}-${row.salesmanName}`}>
                      {index === 0 ? <th className={styles.locationCell} rowSpan={rows.length}>{translateUnmapped(location, t)}</th> : null}
                      <td className={styles.salesmanCell}>{row.salesmanName}{!row.isMapped ? <span>{t("monthlySalesReport.unmapped")}</span> : null}</td>
                      <RowCountCells value={row} />
                    </tr>
                  )),
                  <TotalRow key={`${location}-total`} label={`${translateUnmapped(location, t)} - ${t("monthlySalesReport.locationTotal")}`} total={total} />,
                ];
              })}
            </tbody>
            {report ? <tfoot><TotalRow label={t("monthlySalesReport.grandTotal")} total={report.grandTotal} /></tfoot> : null}
          </table>
        </div>
...
function updateDate(field: "dateFrom" | "dateTo", value: string, setFilters: React.Dispatch<React.SetStateAction<ReportFilters>>) {
  setFilters((current) => {
    if (!value) return { ...current, [field]: value };
    const [year, month] = value.split("-").map(Number);
    const lastDay = toDateInput(new Date(year, month, 0));
    return field === "dateFrom"
      ? { ...current, dateFrom: value, dateTo: current.dateTo.startsWith(value.slice(0, 7)) ? current.dateTo : lastDay }
      : { ...current, dateTo: value, dateFrom: current.dateFrom.startsWith(value.slice(0, 7)) ? current.dateFrom : `${value.slice(0, 7)}-01` };
  });
}

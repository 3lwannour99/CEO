...
export function BarChartCard({
  ...
  const chartData = data
    ...
    .sort(
      (left, right) =>
        Number((right as unknown as Record<string, unknown>)[valueKey]) -
        Number((left as unknown as Record<string, unknown>)[valueKey]),
    );
  ...
  return (
    ...
    >
      <ResponsiveContainer width="100%" height="100%" className={styles.chart}>
        <BarChart
          data={chartData}
          margin={{ top: 16, right: 16, bottom: 84, left: 18 }}
          barCategoryGap="8%"
          barGap={2}
        >
